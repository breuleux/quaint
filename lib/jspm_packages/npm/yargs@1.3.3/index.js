...
(function(process) {
  ...
  function Argv(processArgs, cwd) {
    ...
    self.help = function() {
      ...
      var aliasKeys = (Object.keys(options.alias) || []).concat(Object.keys(self.parsed.newAliases) || []);
      ...
      keys.forEach(function(key) {
        ...
        var extra = [type, demanded[key] ? '[required]' : null, options.default[key] !== undefined ? '[default: ' + (typeof options.default[key] === 'string' ? JSON.stringify : String)(options.default[key]) + ']' : null].filter(Boolean).join('  ');
    ...
    function parseArgs(args) {
      ...
      if (demanded._ && argv._.length < demanded._.count) {
        if (demanded._.msg) {
          ...
          fail('Not enough non-option arguments: got ' + argv._.length + ', need at least ' + demanded._.count);
        }
      }
      ...
      if (strict) {
        ...
        Object.keys(argv).forEach(function(key) {
          if (key !== "$0" && key !== "_" && !descriptions.hasOwnProperty(key) && !demanded.hasOwnProperty(key) && !aliases.hasOwnProperty(key)) {
            unknown.push(key);
          }
        });
        if (unknown.length == 1) {
