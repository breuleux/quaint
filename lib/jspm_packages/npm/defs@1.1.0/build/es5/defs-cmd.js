...
(function(process) {
  ...
  var version = "1.1.0";
  var yargs = require("yargs").options("config", {});
  var argv = yargs.argv;
  if (!argv._.length) {
    var usage = ["defs v" + version + "", "", "Usage: defs OPTIONS <file>", "", "Options: ", "  --config  use specified defs-config.js instead of searching for it"].join("\n");
    console.error(usage);
    process.exit(-1);
  }
