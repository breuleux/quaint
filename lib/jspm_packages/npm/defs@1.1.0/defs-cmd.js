...
(function(process) {
  ...
  const yargs = require("yargs").options("config", {});
  const argv = yargs.argv;
  if (!argv._.length) {
    const usage = ["defs v" + version + "", "", "Usage: defs OPTIONS <file>", "", "Options: ", "  --config  use specified defs-config.js instead of searching for it"].join("\n");
    console.error(usage);
    process.exit(-1);
  }
  ...
  function findAndReadConfig() {
    if (argv.config) {
      const config = tryor(function() {
        return String(fs.readFileSync(argv.config));
      }, null);
      if (!config) {
        console.error("error: config file not found <%s>", argv.config);
        process.exit(-1);
      }
      const json = tryor(function() {
